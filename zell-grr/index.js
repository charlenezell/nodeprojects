    ...
    function replaceContents() {
      ...
      cache.forEach(function replaceInFile(file) {
        ...
        renames.forEach(function replaceOnce(rename) {
          ...
          var reved = options.modifyReved ? options.modifyReved(rename.reved,rename.unreved,file.path) : rename.reved;
          var reg=new RegExp('(?:([\'"])'+unreved+'\\1)',"g");
          contents = contents.replace(reg,"$1"+reved+"$1");
          var reg=new RegExp('(?:\\\('+unreved+'\\\))',"g");
          contents = contents.replace(reg,"("+reved+")");
          if(options.dosthwithContent){
            contents=options.dosthwithContent(contents,rename,file);
          }
          if (options.prefix) {
