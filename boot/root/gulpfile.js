...
gulp.task("sprite",["clean"] ,function() {
    var conCss = [];
    return merge(es.concat.apply(null, glob.sync("src/style/img/sprite/*").map(function(a) {
...
gulp.task("resource",["clean"], function() {
    return gulp.src(["src/**/*.{mp3,jpg,png,swf,gif}", "!src/style/img/sprite/**/*"]).pipe(gulp.dest("dest/"));
});

gulp.task("build", ["clean","css", "js", "html", "resource", "sprite","concatCSS"]);

gulp.task("deploy",["build"],function(){
    return merge(gulp.src(["dest/**/*"]).pipe(gulp.dest("<%= deployRootPath%><%= siteName%>/<%= activityPath%><%= sitePath%>dest/"))
        );
});
